...
kubectl --kubeconfig /certs/config-${USER_NAME} config set-credentials ${USER_NAME} \
--client-certificate=/certs/${USER_NAME}.crt --client-key=/certs/${USER_NAME}.pem --embed-certs=true
echo

echo Setting the context ${CLUSTER_NAME}-${USER_NAME} for user ${USER_NAME} and \
cluster ${CLUSTER_NAME} in /certs/config-${USER_NAME}
